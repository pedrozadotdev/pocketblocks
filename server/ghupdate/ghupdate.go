...
// PocketBlocks executable with the latest GitHub release.
//
// Example usage:
//
...
type Config struct {
	...
	// (default to "pocketblocks"; an additional ".exe" check is also performed as a fallback).
...
func Register(app core.App, rootCmd *cobra.Command, config Config) error {
	...
	if p.config.Repo == "" {
		p.config.Repo = "pocketblocks"
	}

	if p.config.ArchiveExecutable == "" {
		p.config.ArchiveExecutable = "pocketblocks"
	}
...
func (p *plugin) updateCmd() *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "update",
		Short: "Automatically updates the current PocketBlocks executable with the latest available version",
...
func (p *plugin) update(withBackup bool) error {
	...
	if compareVersions(strings.TrimPrefix(p.currentVersion, "v"), strings.TrimPrefix(latest.Tag, "v")) <= 0 {
		color.Green("You already have the latest PocketBlocks %s.", p.currentVersion)
		return nil
	}
	...
	if latest.Body != "" {
		...
		releaseNotes := strings.TrimSpace(strings.Replace(latest.Body, "> _To update the prebuilt executable you can run `./pocketblocks update`._", "", 1))
